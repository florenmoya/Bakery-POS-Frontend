                    ...
                    <v-layout justify-space-between>
                        <v-flex xs6 md9>
                            <v-container class="datatable mt-2 mb-2">
                                <Cart :headers="headers" :items="bali_cart" :single-select="singleSelect" :items_per_page="items_per_page" :dialogShowprop="dialogShowprop" :loading="isLoading" :cart_name="cart_name" />
                            </v-container>
                        </v-flex>
...
export default {
    ...
    data() {
        return {
            tab: null,
            tabs: [
                { name: 'Bali', path: '/sales/bali' }, { name: 'Items', path: '/sales/items' }
            ],
            snack: false,
            snackColor: '',
            snackText: '',
    ...
    mounted() {
        ...
        if (localStorage.getItem('bali_cart')) this.bali_cart = JSON.parse(localStorage.getItem(this.cart_name));
    },
    computed: {
        ...mapGetters({
